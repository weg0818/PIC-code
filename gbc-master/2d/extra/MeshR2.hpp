...
namespace gbc {
    ...
    public:
        ...
        void initializeBarycentricCoordinates(const std::vector< std::vector<double> > &bb)
        {
            ...
            assert(bb.size() == numV);
            const int numC = bb[0].size();
            
            for (int i = 0; i < numV; ++i) {

                _v[i].b().resize(numC);
                for (int j = 0; j < numC; ++j) _v[i].b()[j] = bb[i][j];
            }
        }

        // Get a one ring neighbourhood around the vertex.
        void getRing(const int vInd, std::vector<int> &neighs) const {
        ...
        int getFaceNeighbours(const int faceInd, std::vector<int> &neighs) const {
            ...
            if (heInd >= 0 && _he[heInd].neigh != -1) neighs.push_back((int) heInd);
        ...
        // Subdivide mesh.
        inline void subdivide(const int timesToSubdivide, const bool midpoint = false) {
    ...
    protected:
        ...
        void buildHEConnectivity() {
            ...
            std::vector<halfedgeList> halfedgeTable((int) numV);
