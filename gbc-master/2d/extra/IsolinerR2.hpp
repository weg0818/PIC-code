...
namespace gbc {
    ...
    public:
        ...
        // Get edges.
        inline bool getEdges(const long ind, std::pair<int, int> &e1, std::pair<int, int> &e2) {
    ...
    public:
        // Constructor.
        IsolinerR2(const std::vector<VertexR2> &v, const std::vector<Face> &f, const int coordInd = 0)
    ...
    private:
        ...
        void connectIsopieces(const std::vector<VertexR2> &p, std::vector<std::list<VertexR2> > &result) const {
            ...
            for (int i = 0; i < p.size(); i += 2) pool.push_back(std::make_pair(p[i], p[i + 1]));
