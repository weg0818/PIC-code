        ...
        void computeCoordinates(std::vector<std::vector<double> > &bb) {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                if (p[i].type != INTERIOR) {
                    ...
                    for (int j = 0; j < n; ++j) boundary(indices[i], j) = p[i].b()[j];
                }
            }
