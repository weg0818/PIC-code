...
namespace gbc {
    ...
    public:
        ...
        void evaluate(const VertexR2 &p, std::vector<double> &b) const {
            ...
            const int numF = _meshFaces.cols();
        ...
        void evaluate(const std::vector<VertexR2> &p, std::vector<std::vector<double> > &bb) const {
            ...
            bb.resize(numP);
            for (int i = 0; i < numP; ++i) evaluate(p[i], bb[i]);
        }
        ...
        void evaluate(std::vector<VertexR2> &p) const {
            ...
            const int numP = p.size();
            for (int i = 0; i < numP; ++i) evaluate(p[i], p[i].b());
        }
    ...
    private:
        ...
        void wrapCoordinates(std::vector<std::vector<double> > &bb) const {
            ...
            assert(_v.size() == (int) cols);
            for (long i = 0; i < rows; ++i) {

                bb[i].resize((int) cols);
        ...
        void wrapVertices(std::vector<VertexR2> &p) const {
            ...
            const int n = _v.size();
            const int N = _meshVertices.cols();

            p.clear();
            p.resize(N);

            assert(N == (int) _b.rows());
            assert(_v.size() == (int) _b.cols());
            ...
            for (int i = 0; i < N; ++i) {
                ...
                p[i].b().resize(n, 0.0);
                for (int j = 0; j < n; ++j) p[i].b()[j] = _b(i, j);
            }
        }
        ...
        void refinePolygon(std::vector<VertexR2> &poly) const {
            ...
            for (int i = 0; i < numV; ++i) {
                ...
                const int ip = (i + 1) % numV;
                const int numS = ceil((_v[ip] - _v[i]).length() / _edgeLength);
        ...
        void setTriangulation(const std::vector<VertexR2> &tp, const std::vector<Face> &tf) {
            ...
            const int numP = tp.size();
            const int numF = tf.size();
