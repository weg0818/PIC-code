#include "MaximumLikelihoodCoordinatesR3.h"
#include "../Basic/GBCmath.h"

#include <igl/vertex_triangle_adjacency.h>

void gbc::MaximumLikelihoodCoordinatesR3::compute(std::vector<VertexR3> &p)
{
    Basic::vecVertexToMatXd(_v, cage_v);
    Basic::vecFaceToMatXi(_f, cage_f);
    Basic::vecVertexToMatXd(p, model_v);
    calculateMaximumLikelihoodCoordinates();
    ...
    {
        std::cout << "No calculation of mlc" << std::endl;
    }

}

void gbc::MaximumLikelihoodCoordinatesR3::calculateMaximumLikelihoodCoordinates()
{
    int nv = cage_v.cols();
    int nf = cage_f.cols();
...
void gbc::MaximumLikelihoodCoordinatesR3::computeIntegralUnitNormals(const Eigen::MatrixXd &v,  const Eigen::MatrixXi &f, Eigen::MatrixXd &transMatrix, Eigen::MatrixXd &normalized_integral_outward_allfaces)
{
...
Eigen::VectorXd gbc::MaximumLikelihoodCoordinatesR3::f_gradient(Eigen::VectorXd x, Eigen::MatrixXd v)
{
    // int n = v.cols();
...
Eigen::MatrixXd gbc::MaximumLikelihoodCoordinatesR3::f_Hessian(Eigen::VectorXd x, Eigen::MatrixXd v)
{
